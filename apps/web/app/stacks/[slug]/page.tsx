...
import { daysSince, evidenceConfidence, STALE_EVIDENCE_DAYS } from "@/lib/stack-metadata";
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {safeStack.compounds.length > 0 && (
        <section className="mb-8">
          <h2 className="text-base font-semibold mb-1">Compounds</h2>
          <p className="text-xs text-muted-foreground mb-3">Not medical advice. This view intentionally omits dose/cycle instructions and highlights evidence confidence plus risk/legal caveats.</p>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden md:table-cell">Freshness</TableHead>
                <TableHead className="hidden sm:table-cell">Caveats</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                <TableRow key={sc.id}>
                  ...
                  <TableCell className="text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">{evidenceConfidence(sc.compound.evidenceScore)}</span>
                    </div>
                  </TableCell>
                  <TableCell className="hidden md:table-cell text-xs text-muted-foreground">
                    {(() => {
                      const age = daysSince(sc.compound.lastReviewedAt ?? sc.compound.lastResearchSync);
                      if (age == null) return "Unknown";
                      return age >= STALE_EVIDENCE_DAYS ? `${age}d old (stale)` : `${age}d old`;
                    })()}
                  </TableCell>
                  <TableCell className="hidden sm:table-cell text-xs">
                    <div className="flex flex-wrap gap-1">
                      {(sc.compound.safetyCaveats ?? []).slice(0, 1).map((c) => (
                        <span key={c} className="inline-flex rounded-full bg-amber-500/10 text-amber-700 px-2 py-0.5">{c}</span>
                      ))}
                      {(sc.compound.legalCaveats ?? []).slice(0, 1).map((c) => (
                        <span key={c} className="inline-flex rounded-full bg-rose-500/10 text-rose-700 px-2 py-0.5">{c}</span>
                      ))}
                      {(sc.compound.safetyCaveats?.length ?? 0) + (sc.compound.legalCaveats?.length ?? 0) === 0 && (
