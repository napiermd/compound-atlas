...
function normalizeDetailDescription(raw: string | null, compounds: string[]): string | null {
  const value = raw?.trim();
  if (!value) return null;
  if (value.startsWith("Auto-generated based on evidence scores")) {
    const top = compounds.slice(0, 6).join(", ");
    return `Template stack generated from evidence-ranked compounds${top ? `: ${top}` : ""}. Use as a starting point and validate protocol details before execution.`;
  }
  return value;
}
...
export default async function StackDetailPage({ params }: Props) {
  ...
  const displayDescription = normalizeDetailDescription(
    safeStack.description,
    safeStack.compounds.map((c) => c.compound.name)
  );
  const isUnvalidatedTemplate = isAutoGenerated && (safeStack._count.cycles ?? 0) === 0 && (safeStack.upvotes ?? 0) === 0;
        ...
        <div className="flex flex-col gap-2 mb-6">
          ...
          {isAutoGenerated && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              Auto-generated template from evidence ranking
            </div>
          )}
          {isUnvalidatedTemplate && (
            <div className="flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
              ...
              <span>
                No community runs, forks, or upvotes yet. Treat this as a draft template, not a validated protocol.
              </span>
            </div>
          )}
        </div>
