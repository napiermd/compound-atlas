...
export default async function StackDetailPage({ params }: Props) {
  ...
  const isAnabolicOrHypertrophyGoal =
    safeStack.goal === "BULK" ||
    safeStack.goal === "RECOMP" ||
    safeStack.goal === "ATHLETIC_PERFORMANCE" ||
    safeStack.goal === "HORMONE_OPTIMIZATION";
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {(variantExplainer || isAutoGenerated || isAnabolicOrHypertrophyGoal) && (
          ...
          {isAnabolicOrHypertrophyGoal && (
            <div className="flex items-start gap-2 rounded-md border border-blue-500/30 bg-blue-500/5 px-3 py-2">
              ...
              <p className="text-xs text-muted-foreground">
                How to read anabolic/hypertrophy stacks: {confidenceLabelFromScore(safeStack.evidenceScore ?? 0).toLowerCase()} reflects literature strength for the protocol, not guaranteed individual gains.
              </p>
            </div>
          )}
          {isAutoGenerated && (
